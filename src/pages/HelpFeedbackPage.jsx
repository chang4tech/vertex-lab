...
import { helpFeedbackContacts } from '../config/helpLinks.js';
import helpFeedbackTemplate from '../templates/helpFeedback.html?raw';
...
const makeMailAnchor = (email) => {
  ...
  return `<a href="${href}">${safeEmail}</a>`;
};

const buildTemplate = () => {
  let html = helpFeedbackTemplate;
  const { ideas, bugs, security, note } = helpFeedbackContacts;
  const replacements = {
    '{{IDEAS_ANCHOR}}': makeMailAnchor(ideas?.email),
    '{{IDEAS_DESC}}': escapeHtml(ideas?.description || ''),
    '{{BUGS_ANCHOR}}': makeMailAnchor(bugs?.email),
    '{{BUGS_DESC}}': escapeHtml(bugs?.description || ''),
    '{{SECURITY_ANCHOR}}': makeMailAnchor(security?.email),
    '{{SECURITY_DESC}}': escapeHtml(security?.description || ''),
